...
function EpisodeCard({
  ...
  const epImg     = EP_IMAGES[ep.number];
  ...
  return (
    ...
    >
      {/* Thumbnail */}
      <div style={{
        width: 'clamp(120px, 163px, 163px)', height: 'clamp(70px, 93px, 93px)',
...
/* ─── Section header (STANDART / PREMIUM) ─── */
...
export default function HomePage() {
  ...
  const continueEp = standardEps.find(e => { const p = e.progress || 0; return p > 0 && p < 100; }) || standardEps[0];
  const continueProgress = continueEp ? (continueEp.progress || 0) : 34;

  function handleEpisodeClick(ep: typeof EPISODES[number]) {
    if (ep.tier === 'premium' && !isPremiumUser) {
      router.push('/trailer');
      return;
    }
  ...
  return (
    <div style={{ fontFamily: 'Poppins, sans-serif', position: 'relative' }}>
      ...
      <div style={{
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
              <div style={{ flex: 1, height: '5px', background: '#470d0d', borderRadius: '5px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${continueProgress}%`, background: '#fff', borderRadius: '5px' }} />
              </div>
              <span style={{ fontSize: '9px', fontWeight: 700, color: '#fff', flexShrink: 0 }}>{continueProgress}%</span>
            </div>
            <button style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)', marginBottom: 'clamp(24px, 4vw, 36px)' }}>
          {standardEps.map(ep => (
            <EpisodeCard
              key={ep.id}
              ep={ep}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
          ...
          {/* If no premium episodes exist in data, show placeholder */}
          {premiumEps.length === 0 && standardEps.map(ep => (
            <EpisodeCard
              key={`prem-${ep.id}`}
              ep={{ ...ep, id: `prem-${ep.id}`, tier: 'premium' as const }}
              isLocked={!isPremiumUser}
